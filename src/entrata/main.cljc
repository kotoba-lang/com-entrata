(ns entrata.main
  "Kotodama WASM entrypoint for the Entrata clean-room actor (L4) — Clojure port.
  ...
  facts (`entrata.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "entrata")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "entrata_lis"
   ...
   {:entity "Tenant"      :plural "tenants"      :id-prefix "entrata_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"entrata.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "entrata-compat" :tier tier :entities entities} 200])
...
(defn start! [] :entrata-compat/ready)
